...
`
    ...
    useEffect(() => {
        if (typeof token === "object" && token.kind === "storing") {
            window.api.savePersonalAccessToken(token.token).then(() => {
    ...
    if (typeof token !== 'object' || !(token.kind === "entered" || token.kind === "restored")) {
        ...
        throw new Error("This should never happen. The token should be an object with kind 'entered' or 'restored' at this point.")
    }
